...
class WekanCard(WekanBase):
    def __init__(self, parent_list: WekanList, card_id: str) -> None:
        ...
        self.date_last_activity = self.list.board.client.parse_iso_date(
    ...
    def get_checklists(self, regex_filter=".*") -> list[CardChecklist]:
        ...
        """
        all_checklists = CardChecklist.from_list(
            parent_card=self, data=self.__get_all_checklists()
        )
        return [
            checklist
            for checklist in all_checklists
            if re.search(regex_filter, checklist.title)
        ]
    ...
    def add_comment(self, text: str) -> dict:
        ...
        response = self.list.board.client.fetch_json(
            uri, http_method="POST", payload=payload
        )
        return response
