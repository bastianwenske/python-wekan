...
def show() -> None:
    ...
    panel_content.append(
        f" Config file: {config_file or 'Not found (using defaults/env vars)'}"
    )
...
def set(
    ...
    timeout: Optional[int] = typer.Option(
        None, "--timeout", "-t", help="Request timeout (ms)"
    ),
    config_file: Optional[str] = typer.Option(
        None, "--file", "-f", help="Config file path"
    ),
    ...
    # Determine config file path
    config_path = (
        Path(config_file) if config_file else (find_config_file() or Path(".wekan"))
    )
