...
def list_boards() -> None:
    ...
    try:
        ...
        console.print(table)
        console.print(
            "\n Use board [bold]index (#)[/bold], [bold]ID prefix[/bold], or [bold]name[/bold] with other commands"
        )
...
def show(identifier: str) -> None:
    ...
    try:
        ...
        panel_content.append(
            f" Description: {getattr(board, 'description', 'No description')}"
        )
        ...
        if lists:
            ...
            for i, lst in enumerate(lists, 1):
                ...
                list_table.add_row(
                    str(i), lst.id[:8] + "...", f"{lst.title} ({card_count} cards)"
                )
