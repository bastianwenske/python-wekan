...
class BoardContext:
    ...
    def show_board(self):
        ...
        try:
            ...
            try:
                lists = (
                    self.board.get_lists() if hasattr(self.board, "get_lists") else []
                )
            except Exception as e:
                self.console.print(
                    f"[yellow]  Could not load board lists: {str(e)}[/yellow]"
                )
                lists = []
                ...
                if cards:
                    ...
                    if len(cards) > 10:
                        cards_content.append(
                            f"[dim]... and {len(cards) - 10} more[/dim]"
                        )
                else:
            ...
            board_info = f" [bold blue]{self.board.title}[/bold blue] | Lists: {len(lists)} | ID: {self.board.id[:8]}..."
            self.console.print(
                Panel.fit(board_info, title="Active Board", border_style="green")
            )
    ...
    def run_interactive_session(self):
        ...
        self.console.print(
            "Type [bold]help[/bold] for available commands, [bold]exit[/bold] to leave board context"
        )
    ...
    def show_board_info(self):
        ...
        try:
            ...
            swimlanes = (
                self.board.list_swimlanes()
                if hasattr(self.board, "list_swimlanes")
                else []
            )
    ...
    def handle_cards_command(self, args):
        ...
        try:
            ...
            if list_id.isdigit():
                ...
                for lst in lists:
                    if (
                        lst.id.startswith(list_id)
                        or list_id.lower() in lst.title.lower()
                    ):
                        target_list = lst
                        break
            ...
            if not cards:
                self.console.print(
                    f"[yellow] No cards in list '{target_list.title}'[/yellow]"
                )
                return
