...
def main_callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        ...
        console.print(
            "  • [bold cyan]wekan status[/bold cyan] - Show connection status"
        )
        console.print(
            "  • [bold cyan]wekan navigate[/bold cyan] - Start navigation shell (recommended!)"
        )
        console.print(
            "  • [bold cyan]wekan config init[/bold cyan] - Initialize configuration"
        )
        ...
        console.print(
            "  • [bold green]boards[/bold green]  - Board management commands"
        )
        console.print(
            "  • [bold green]config[/bold green]  - Configuration management commands"
        )
        console.print()
        console.print(
            "Use [bold]wekan --help[/bold] for detailed help or [bold]wekan <command> --help[/bold] for command-specific help"
        )
        console.print()
...
def status():
    ...
    try:
        ...
        if not config.base_url:
            console.print(
                "No WeKan server configured. Run 'wekan config init' to set up."
            )
            raise typer.Exit(1)

        if not config.username or not config.password:
            console.print(
                "No credentials configured. Run 'wekan config init' to set up."
            )
            raise typer.Exit(1)
