...
class Board(WekanBase):
    def __init__(self, client: WekanClient, board_id: str) -> None:
        ...
        self.allows_card_counterList = self.__raw_data.get(
            "allowsCardCounterList", None
        )
        self.allows_board_member_list = self.__raw_data.get(
            "allowsBoardMemberList", None
        )
        self.date_settings_default_board_id = self.__raw_data.get(
            "dateSettingsDefaultBoardId", None
        )
        self.date_settings_default_list_id = self.__raw_data.get(
        ...
        self.allows_card_sorting_by_number = self.__raw_data.get(
            "allowsCardSortingByNumber", None
        )
    ...
    def list_custom_fields(self, regex_filter=".*") -> list[Customfield]:
        ...
        return [
            field for field in all_custom_fields if re.search(regex_filter, field.name)
        ]
    ...
    def get_labels(self, regex_filter=".*") -> list[Label]:
        ...
        """
        all_labels = Label.from_list(
            parent_board=self, data=self.__raw_data.get("labels", [])
        )
    ...
    def list_swimlanes(self, regex_filter=".*") -> list[Swimlane]:
        ...
        """
        all_swimlanes = Swimlane.from_list(
            parent_board=self, data=self.__get_all_swimlanes()
        )
        return [
            swimlane
            for swimlane in all_swimlanes
            if re.search(regex_filter, swimlane.title)
        ]
    ...
    def get_swimlane_by_id(self, swimlane_id: str) -> Swimlane:
        ...
        response = self.client.fetch_json(
            f"/api/boards/{self.id}/swimlanes/{swimlane_id}"
        )
    ...
    def get_integration_by_id(self, integration_id: str) -> Integration:
        ...
        """
        response = self.client.fetch_json(
            f"/api/boards/{self.id}/integrations/{integration_id}"
        )
    ...
    def get_custom_field_by_id(self, custom_field_id: str) -> Customfield:
        ...
        """
        response = self.client.fetch_json(
            f"/api/boards/{self.id}/custom-fields/{custom_field_id}"
        )
    ...
    def add_custom_field(
        ...
        show_sum_at_top_of_list = (
            show_sum_at_top_of_list if field_type == "currency" else False
        )
    ...
    def update(
        ...
        if payload:
            self.client.fetch_json(
                f"/api/boards/{self.id}", http_method="PUT", payload=payload
            )
            # Refresh data
