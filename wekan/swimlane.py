...
class Swimlane(WekanBase):
    def __init__(self, parent_board: Board, swimlane_id: str) -> None:
        ...
        data = self.board.client.fetch_json(
            f"/api/boards/{self.board.id}/swimlanes/{self.id}"
        )
        self.title = data["title"]
        self.archived = data["archived"]
        self.created_at = self.board.client.parse_iso_date(data["createdAt"])
        self.updated_at = self.board.client.parse_iso_date(
            data.get("updatedAt", data["createdAt"])
        )
    ...
    def from_list(cls, parent_board: Board, data: list) -> list[Swimlane]:
        ...
        for swimlane in data:
            instances.append(
                cls(parent_board=parent_board, swimlane_id=swimlane["_id"])
            )
        return instances
